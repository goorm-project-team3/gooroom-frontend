      ...
      <div className="p-6" style={{ 
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            ...
            <input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="example@email.com" required style={inputStyle} onFocus={handleFocus} onBlur={handleBlur} />
          </div>

          {/* 비밀번호 */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <label style={{ fontSize: '13px', fontWeight: '600', color: '#475569' }}>비밀번호</label>
            <input type="password" name="password" value={formData.password} onChange={handleChange} placeholder="6자리 이상 20자리 이하" required style={inputStyle} onFocus={handleFocus} onBlur={handleBlur} />
          </div>

          {/* 비밀번호 확인 */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <label style={{ fontSize: '13px', fontWeight: '600', color: '#475569' }}>비밀번호 확인</label>
            <input type="password" name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} placeholder="비밀번호를 한번 더 입력" required style={inputStyle} onFocus={handleFocus} onBlur={handleBlur} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            ...
            <input type="text" name="username" value={formData.username} onChange={handleChange} placeholder="홍길동" required style={inputStyle} onFocus={handleFocus} onBlur={handleBlur} />
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            ...
            <div style={{ 
              ...
              <label style={{ 
                ...
                />
                <span style={{ fontSize: '15px', fontWeight: formData.role === 'OWNER' ? '700' : '500', color: formData.role === 'OWNER' ? '#1e40af' : '#334155' }}>
                  강사 (OWNER)
                </span>
              </label>
              ...
              <label style={{ 
                ...
                />
                <span style={{ fontSize: '15px', fontWeight: formData.role === 'USER' ? '700' : '500', color: formData.role === 'USER' ? '#1e40af' : '#334155' }}>
                  학생 (USER)
                </span>
              </label>
            </div>
          ...
          <button
            ...
            style={{
              ...
              cursor: isBtnDisabled ? 'not-allowed' : 'pointer', marginTop: '8px',
...
const inputStyle: React.CSSProperties = {
  padding: '13px 16px', border: '1px solid #cbd5e1', borderRadius: '10px', fontSize: '15px',
  color: '#334155', outline: 'none', backgroundColor: '#f8fafc', transition: 'all 0.2s ease-in-out'
};